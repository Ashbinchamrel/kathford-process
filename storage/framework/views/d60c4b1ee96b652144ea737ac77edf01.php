...
<?php $__env->startSection('page-title', 'Edit Purchase Order'); ?>
...
<div class="max-w-4xl space-y-4">
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-800">
        Editing <span class="font-mono font-semibold"><?php echo e($purchaseOrder->po_number); ?></span>
        — Vendor: <strong><?php echo e($purchaseOrder->vendor->name); ?></strong>
    </div>

    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('purchase_orders.edit')): ?>
<form method="POST" action="<?php echo e(route('purchase-orders.update', $purchaseOrder)); ?>">
        <?php echo csrf_field(); ?>
<input type="hidden" name="_fiscal_year_id" value="<?php echo e($workingFiscalYear?->id); ?>">
        <?php echo method_field('PUT'); ?>

        
        <?php if(! $purchaseOrder->rfq_quote_id): ?>
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div id="items-container" class="space-y-2">
                ...
                <div class="grid grid-cols-12 gap-2 items-start item-row">
                    <div class="col-span-3">
                        <input type="text" name="items[<?php echo e($loop->index); ?>][description]" value="<?php echo e($item->description); ?>" required
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                    </div>
                    <div class="col-span-2">
                        <input type="number" name="items[<?php echo e($loop->index); ?>][quantity]" value="<?php echo e($item->quantity); ?>" step="0.01" min="0.01" required
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none qty-field">
                    </div>
                    <div class="col-span-2">
                        <input type="text" name="items[<?php echo e($loop->index); ?>][unit]" value="<?php echo e($item->unit); ?>"
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                    </div>
                    <div class="col-span-2">
                        <input type="number" name="items[<?php echo e($loop->index); ?>][unit_rate]" value="<?php echo e($item->unit_rate); ?>" step="0.01" min="0" required
                    ...
                    <div class="col-span-2 flex items-center justify-end px-2 font-mono text-sm font-semibold text-gray-700 line-amount">Rs <?php echo e(number_format($item->quantity * $item->unit_rate, 2)); ?></div>
            ...
            <div class="flex flex-wrap justify-end gap-x-5 gap-y-2 pt-3 border-t border-gray-100 text-sm">
                <span class="text-gray-500">Total Amount: <span id="subtotal-display" class="font-mono font-semibold text-gray-800">Rs <?php echo e(number_format($purchaseOrder->subtotal, 2)); ?></span></span>
                <span class="text-gray-500">Tax Amount: <span id="tax-display" class="font-mono font-semibold text-gray-800">Rs <?php echo e(number_format($purchaseOrder->tax_amount, 2)); ?></span></span>
                <span class="font-semibold text-teal-800">Grand Total: <span id="grand-total-display" class="font-mono">Rs <?php echo e(number_format($purchaseOrder->total_amount, 2)); ?></span></span>
            </div>
        </div>
        <?php else: ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <textarea name="delivery_address" rows="2" required
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none resize-none"><?php echo e(old('delivery_address', $purchaseOrder->delivery_address)); ?></textarea>
                </div>
                <div>
                    ...
                    <input type="date" name="expected_delivery_date"
                           value="<?php echo e(old('expected_delivery_date', $purchaseOrder->expected_delivery_date?->format('Y-m-d'))); ?>" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                </div>
                <div class="sm:col-span-2 flex flex-wrap items-center gap-4 rounded-lg bg-teal-50 p-3"><label class="flex items-center gap-2 text-sm font-medium text-teal-900"><input type="hidden" name="tax_applied" value="0"><input type="checkbox" name="tax_applied" value="1" <?php if(old('tax_applied', $purchaseOrder->tax_applied)): echo 'checked'; endif; ?> class="rounded border-teal-300 text-teal-600"> Apply tax / VAT</label><label class="text-sm text-teal-900">Tax rate (%) <input type="number" name="tax_rate" value="<?php echo e(old('tax_rate', $purchaseOrder->tax_rate ?: 13)); ?>" min="0" max="100" step="0.01" class="ml-1 w-24 rounded border border-teal-200 bg-white px-2 py-1 text-right text-sm"></label><p class="text-xs text-teal-700">Tax is calculated from the PO gross amount when saved.</p></div>
                <div class="sm:col-span-2">
                    ...
                    <textarea name="terms_and_conditions" rows="3" required
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none resize-none"><?php echo e(old('terms_and_conditions', $purchaseOrder->terms_and_conditions)); ?></textarea>
                </div>
            </div>
        </div>
...
<a href="<?php echo e(route('purchase-orders.show', $purchaseOrder)); ?>" class="text-gray-500 hover:text-gray-700 text-sm py-2.5">Cancel</a>
<?php endif; ?>
        </div>
    </form>
<?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ashbinkumarchamrel/Downloads/kathford-process/resources/views/purchase-orders/edit.blade.php ENDPATH**/ ?>