<?php $__env->startSection('title', $form->form_number); ?>
<?php $__env->startSection('page-title', $form->form_number); ?>
...
<div class="max-w-4xl space-y-5" x-data="{ previewUrl: null, previewName: '' }">
    ...
    <div class="rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700"><?php echo e(session('error')); ?></div>
    <?php endif; ?>
    ...
    <div class="rounded-xl border px-5 py-4 <?php echo e($bannerClass); ?>">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <div class="text-right flex-shrink-0">
                ...
                <p class="font-mono font-bold text-lg"><?php echo e($form->form_number); ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-sm">
            <div class="sm:col-span-2">
                ...
                <dd class="text-gray-900 font-medium text-base"><?php echo e($form->activity_name); ?></dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400 font-semibold uppercase mb-1">Requested By</dt>
                <dd class="text-gray-700"><?php echo e($form->creator?->name ?? '—'); ?></dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400 font-semibold uppercase mb-1">Department</dt>
                <dd class="text-gray-700"><?php echo e($form->department?->name ?? '—'); ?></dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400 font-semibold uppercase mb-1">Category</dt>
                <dd class="text-gray-700"><?php echo e($form->category?->name ?? '—'); ?></dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400 font-semibold uppercase mb-1">Required By (Deadline)</dt>
                <dd class="text-gray-700"><?php echo e($form->deadline_date ? $form->deadline_date->format('d M Y') : '—'); ?></dd>
            </div>
            <?php if(true): ?>
            <div class="sm:col-span-2">
                <dt class="text-xs text-gray-400 font-semibold uppercase mb-1">Remarks</dt>
                <dd class="text-gray-700"><?php echo e($form->remarks ?? '—'); ?></dd>
            </div>
            <?php endif; ?>
            <?php if($form->unplanned_reason): ?>
            <div class="sm:col-span-2">
                <dt class="text-xs text-gray-400 font-semibold uppercase mb-1">Reason (Unplanned)</dt>
                <dd class="text-gray-700"><?php echo e($form->unplanned_reason); ?></dd>
            </div>
            <?php endif; ?>
        </dl>
    </div>
    ...
    ?>
    <div class="bg-white rounded-xl border <?php echo e($variance > 0 ? 'border-amber-300' : 'border-teal-200'); ?> p-6">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
            <div>
                <h2 class="font-semibold text-gray-800 text-sm uppercase tracking-wide">Department Budget</h2>
                <p class="text-sm text-gray-600 mt-1"><?php echo e($form->budget->activity_title); ?> · FY <?php echo e($form->budget->fiscal_year); ?></p>
            </div>
            <?php if($variance > 0): ?>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            <div><p class="text-xs uppercase font-semibold text-gray-400">Allocation</p><p class="font-semibold mt-1">Rs <?php echo e(number_format($form->budget->allocated_amount, 2)); ?></p></div>
            <div><p class="text-xs uppercase font-semibold text-gray-400">Current commitments</p><p class="font-semibold mt-1">Rs <?php echo e(number_format($reservedBeforeThisForm, 2)); ?></p></div>
            <div><p class="text-xs uppercase font-semibold text-gray-400">Remaining balance</p><p class="font-semibold mt-1">Rs <?php echo e(number_format($remainingBeforeThisForm, 2)); ?></p></div>
            <div><p class="text-xs uppercase font-semibold text-gray-400">This request</p><p class="font-semibold mt-1">Rs <?php echo e(number_format($form->total_estimated_amount, 2)); ?></p></div>
        </div>
        <?php if($variance > 0): ?>
            <p class="mt-4 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-sm text-amber-800">This request is Rs <?php echo e(number_format($variance, 2)); ?> above the current remaining balance. It is recorded as a forecast only; the normal approval process is unchanged.</p>
        <?php endif; ?>
    </div>
    <?php endif; ?>

    
    <?php if($form->approvalChain): ?>
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-2"><h2 class="font-semibold text-gray-800 text-sm uppercase tracking-wide">Approval Chain — <?php echo e($form->approvalChain->name); ?></h2><?php if($step = app(\App\Services\ApprovalService::class)->pendingStepLabel($form)): ?><span class="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800">Pending: <?php echo e($step); ?></span><?php endif; ?></div>
        <?php
        ...
        ?>
        <?php if($remainingVerifiers->isNotEmpty() || $remainingApprovers->isNotEmpty()): ?><div class="mb-4 rounded-lg bg-amber-50 p-3 text-sm"><strong>Remaining authorities:</strong> <?php echo e($remainingVerifiers->merge($remainingApprovers)->pluck('name')->implode(' → ')); ?></div><?php endif; ?>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div>
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Creator (Layer 1)</p>
                <p class="text-sm font-medium text-gray-800"><?php echo e($form->creator?->name ?? 'Former user'); ?></p>
                <p class="text-xs text-gray-500">Created <?php echo e($form->created_at?->format('d M Y H:i')); ?></p>
            </div>
            <div>
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Verifiers (Layer 2)</p>
                <?php $__empty_1 = true; $__currentLoopData = $form->approvalChain->verifiers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
...
?>
                        <?php $__empty_2 = true; $__currentLoopData = $authorityActions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $authorityAction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                            <p class="text-xs text-gray-600"><?php echo e(ucfirst(str_replace('_', ' ', $authorityAction->decision))); ?> · <?php echo e($authorityAction->acted_at?->format('d M Y H:i')); ?></p>
            ...
            <div>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $form->approvalChain->approvers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="flex items-center gap-2 mb-2">
...
?>
    ...
    <?php if($form->lineItems->isNotEmpty()): ?>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php $__currentLoopData = $form->lineItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="hover:bg-gray-50">
                        <td class="px-5 py-3 text-gray-400"><?php echo e($idx + 1); ?></td>
                        <td class="px-5 py-3 text-gray-800 font-medium"><?php echo e($item->item_name); ?></td>
                        ...
                        <td class="px-5 py-3 text-right font-mono"><?php echo e(number_format($item->rate, 2)); ?></td>
                        <td class="px-5 py-3 text-right font-mono font-semibold"><?php echo e(number_format($item->amount, 2)); ?></td>
                    </tr>
                ...
                <tfoot class="bg-gray-50">
                    <tr>
                        ...
                        <td class="px-5 py-3 text-right font-mono font-bold text-gray-900 text-base">Rs <?php echo e(number_format($form->total_estimated_amount, 2)); ?></td>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    <?php if($form->attachments->isNotEmpty()): ?>
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h2 class="font-semibold text-gray-800 mb-3 text-sm uppercase tracking-wide">Attachments</h2>
        <ul class="space-y-2">
            <?php $__currentLoopData = $form->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $att): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <li class="flex items-center gap-3 text-sm">
                ...
                <?php if($att->external_link): ?>
                <a href="<?php echo e($att->external_link); ?>" class="text-teal-600 hover:underline flex-1" target="_blank" rel="noopener noreferrer"><?php echo e($att->original_name ?? $att->external_link); ?></a>
                <?php else: ?>
                <?php
...
?>
                ...
                <button type="button"
                    @click="previewUrl='<?php echo e(route('activity-forms.attachment.download', [$form, $att])); ?>'; previewName='<?php echo e(addslashes($att->original_name)); ?>'"
                    class="text-teal-600 hover:underline flex-1 text-left"><?php echo e($att->original_name); ?></button>
                <?php else: ?>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('activity_forms.view')): ?>
<a href="<?php echo e(route('activity-forms.attachment.download', [$form, $att])); ?>" class="text-teal-600 hover:underline flex-1" target="_blank" rel="noopener noreferrer"><?php echo e($att->original_name); ?></a>
<?php endif; ?>
                <?php endif; ?>
                <?php endif; ?>
                <span class="text-gray-400 text-xs"><?php echo e($att->humanSize()); ?></span>
                <?php if($form->isEditable() && $form->creator_id === auth()->id()): ?>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('activity_forms.edit')): ?>
<form method="POST" action="<?php echo e(route('activity-forms.attachment.delete', [$form, $att])); ?>" class="inline">
                    <?php echo csrf_field(); ?>
...
<?php endif; ?>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <?php if($form->approvalActions->isEmpty()): ?>
        ...
        <ol class="relative border-l-2 border-gray-100 space-y-5 ml-3">
            <?php $__currentLoopData = $form->approvalActions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $action): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                $dot = [
            ...
            <li class="ml-5">
                ...
                <p class="text-sm font-semibold text-gray-800">
                    ...
                    <span class="font-normal text-gray-600">—
                        <?php echo e($action->decision === 'modified_approved' ? 'Modified & Approved' : ucfirst($action->decision)); ?>
    ...
    <?php if($form->rfqs->isNotEmpty()): ?>
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h2 class="font-semibold text-gray-800 mb-3 text-sm uppercase tracking-wide">Linked RFQs</h2>
        <div class="space-y-2">
            <?php $__currentLoopData = $form->rfqs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rfq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                    <span class="font-mono text-xs font-bold text-gray-700"><?php echo e($rfq->rfq_number); ?></span>
                    <span class="ml-2 text-sm text-gray-600"><?php echo e($rfq->title); ?></span>
                </div>
                <div class="flex items-center gap-3">
                    <span class="text-xs font-medium px-2 py-0.5 rounded-full
                        <?php echo e($rfq->status === 'closed' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'); ?>">
                        <?php echo e(ucfirst(str_replace('_',' ',$rfq->status))); ?>
...
<?php endif; ?>
    ...
    <div class="flex flex-wrap gap-3 items-center">
        ...
        <?php if($form->status === 'draft' && $form->creator_id === auth()->id()): ?>
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('activity_forms.submit')): ?>
<form method="POST" action="<?php echo e(route('activity-forms.submit', $form)); ?>">
            <?php echo csrf_field(); ?>
...
<?php endif; ?>
        ...
        <?php if($form->isEditable() && $form->creator_id === auth()->id()): ?>
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('activity_forms.edit')): ?>
<a href="<?php echo e(route('activity-forms.edit', $form)); ?>" class="btn-secondary">Edit Form</a>
<?php endif; ?>
        <?php endif; ?>

        
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('verify', $form)): ?>
        <button type="button"
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('finalApprove', $form)): ?>
        <button type="button"
        ...
        <?php if($form->isApproved() && (auth()->id() === $form->creator_id || auth()->user()->hasAnyRole(['finance','super_admin']))): ?>
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('rfq.create')): ?>
<a href="<?php echo e(route('rfq.create', ['activity_form_id' => $form->id])); ?>"
           class="px-5 py-2 rounded-lg text-sm font-semibold text-white"
...
<?php endif; ?>
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('activity_forms.view')): ?>
<a href="<?php echo e(route('activity-forms.index')); ?>" class="text-gray-500 hover:text-gray-700 text-sm py-2 ml-auto">← Back to List</a>
<?php endif; ?>
    </div>
</div>
...
<div id="verify-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('activity_forms.verify')): ?>
<form method="POST" action="<?php echo e(route('activity-forms.verify', $form)); ?>">
            <?php echo csrf_field(); ?>
<input type="hidden" name="_fiscal_year_id" value="<?php echo e($workingFiscalYear?->id); ?>">
            <div class="space-y-4">
...
<div id="approve-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('activity_forms.approve')): ?>
<form method="POST" action="<?php echo e(route('activity-forms.approve', $form)); ?>">
            <?php echo csrf_field(); ?>
<input type="hidden" name="_fiscal_year_id" value="<?php echo e($workingFiscalYear?->id); ?>">
            <div class="space-y-4">
                <div class="flex gap-4">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ashbinkumarchamrel/Downloads/kathford-process/resources/views/activity-forms/show.blade.php ENDPATH**/ ?>