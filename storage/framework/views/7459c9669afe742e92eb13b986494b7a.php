...
<?php $__env->startSection('page-title', 'Organisation Profile'); ?>
...
<div class="mx-auto max-w-6xl space-y-6">
    <section class="overflow-hidden rounded-2xl bg-[#0B1E3D] px-6 py-6 text-white shadow-sm sm:px-7">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex h-20 w-28 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/15 bg-white/10 p-2">
                <?php if($logoPath): ?>
                    <img src="<?php echo e(asset('storage/'.$logoPath)); ?>" alt="Current organisation logo" class="h-full w-full object-contain">
                <?php else: ?>
    ...
    <?php endif; ?>
    <?php if($errors->any()): ?>
        <div class="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800"><p class="font-semibold">Your changes were not saved. Please correct the highlighted details.</p><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><p class="mt-1"><?php echo e($error); ?></p><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></div>
    <?php endif; ?>

    <?php echo $__env->make('admin.setup.fiscal-years', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

    <form method="POST" action="<?php echo e(route('admin.profile.update')); ?>" enctype="multipart/form-data" class="grid gap-6 lg:grid-cols-5">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white shadow-sm lg:col-span-3">
            ...
            <div class="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
                <div class="sm:col-span-2">
                    ...
                    <input id="company-name" required name="company_name" value="<?php echo e(old('company_name', $company['company_name'] ?? 'Kathford International College')); ?>" placeholder="Organisation legal name" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                </div>
                <div>
                    <label for="company-pan" class="mb-1.5 block text-sm font-semibold text-slate-700">PAN / VAT number</label>
                    <input id="company-pan" name="company_pan" value="<?php echo e(old('company_pan', $company['company_pan'] ?? '')); ?>" placeholder="e.g. 301936424" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                </div>
                <div>
                    <label for="company-phone" class="mb-1.5 block text-sm font-semibold text-slate-700">Phone</label>
                    <input id="company-phone" name="company_phone" value="<?php echo e(old('company_phone', $company['company_phone'] ?? '')); ?>" placeholder="e.g. 01-5201911" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                </div>
                <div>
                    <label for="company-email" class="mb-1.5 block text-sm font-semibold text-slate-700">Official email</label>
                    <input id="company-email" type="email" name="company_email" value="<?php echo e(old('company_email', $company['company_email'] ?? '')); ?>" placeholder="accounts@example.edu.np" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                </div>
                <div>
                    <label for="company-website" class="mb-1.5 block text-sm font-semibold text-slate-700">Website</label>
                    <input id="company-website" type="url" name="company_website" value="<?php echo e(old('company_website', $company['company_website'] ?? '')); ?>" placeholder="https://example.edu.np" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                </div>
                <div class="sm:col-span-2">
                    <label for="company-address" class="mb-1.5 block text-sm font-semibold text-slate-700">Registered address</label>
                    <textarea id="company-address" name="company_address" rows="3" placeholder="Street, municipality, district, country" class="w-full resize-y rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100"><?php echo e(old('company_address', $company['company_address'] ?? '')); ?></textarea>
                </div>
            </div>
        </section>
                ...
                <div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4">
                    <div class="flex items-center gap-4">
                        <div id="logo-preview-shell" class="flex h-20 w-28 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-white p-2">
                            <?php if($logoPath): ?><img id="logo-preview" src="<?php echo e(asset('storage/'.$logoPath)); ?>" alt="Current organisation logo" class="h-full w-full object-contain"><?php else: ?><img id="logo-preview" alt="Logo preview" class="hidden h-full w-full object-contain"><span id="logo-placeholder" class="text-center text-xs text-slate-400">Logo preview</span><?php endif; ?>
                        </div>
                    ...
                    <p id="logo-file-name" class="mt-2 text-center text-xs text-slate-500"><?php echo e($logoPath ? 'Current logo will remain unless you select a replacement.' : 'No file selected.'); ?></p>
                </div>
                <div>
                    <label for="currency-symbol" class="mb-1.5 block text-sm font-semibold text-slate-700">Currency symbol</label>
                    <input id="currency-symbol" name="currency_symbol" value="<?php echo e(old('currency_symbol', $company['currency_symbol'] ?? 'Rs')); ?>" placeholder="Rs" maxlength="10" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                </div>

                <div>
                    <label for="date-format" class="mb-1.5 block text-sm font-semibold text-slate-700">Working date format</label>
                    <select id="date-format" name="date_format" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-100"><option value="AD" <?php echo e(old('date_format', $company['date_format'] ?? 'AD') === 'AD' ? 'selected' : ''); ?>>AD (Gregorian)</option><option value="BS" <?php echo e(old('date_format', $company['date_format'] ?? '') === 'BS' ? 'selected' : ''); ?>>BS (Bikram Sambat)</option></select>
                </div>
            </div>
        </section>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ashbinkumarchamrel/Downloads/kathford-process/resources/views/admin/setup/company.blade.php ENDPATH**/ ?>