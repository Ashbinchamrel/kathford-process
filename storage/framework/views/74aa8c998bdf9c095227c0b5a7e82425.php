<?php $__env->startSection('title', 'Edit PR'); ?>
<?php $__env->startSection('page-title', 'Edit PR: ' . $pr->pr_number); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-4xl" x-data="prBuilder()">
    <form method="POST" action="<?php echo e(route('purchase-requests.update', $pr)); ?>" enctype="multipart/form-data">
        <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-4 space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Title <span class="text-red-500">*</span></label>
                    <input type="text" name="title" value="<?php echo e(old('title', $pr->title)); ?>" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                </div>
                <div>
                    ...
                    <select name="activity_form_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                        ...
                        <?php $__currentLoopData = $activityForms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $af): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($af->id); ?>" <?php echo e(old('activity_form_id', $pr->activity_form_id) == $af->id ? 'selected' : ''); ?>><?php echo e($af->form_number); ?> – <?php echo e($af->title); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Required By</label>
                    <input type="date" name="required_by" value="<?php echo e(old('required_by', $pr->required_by?->format('Y-m-d'))); ?>" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Justification</label>
                    <textarea name="justification" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none resize-none"><?php echo e(old('justification', $pr->justification)); ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="<?php echo e(route('purchase-requests.show', $pr)); ?>" class="text-gray-500 hover:text-gray-700 text-sm py-2.5">Cancel</a>
        </div>
    </form>
</div>
...
<script>
function prBuilder() {
    const initial = <?php echo e(Illuminate\Support\Js::from($pr->lineItems->map(fn($i) => ['description'=>$i->description,'unit'=>$i->unit,'quantity'=>$i->quantity,'unit_rate'=>$i->unit_rate,'amount'=>$i->amount]))); ?>;
    return {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ashbinkumarchamrel/Downloads/kathford-process/resources/views/purchase-requests/edit.blade.php ENDPATH**/ ?>