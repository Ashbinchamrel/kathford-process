...
<input type="hidden" name="_fiscal_year_id" value="<?php echo e($workingFiscalYear?->id); ?>">
        <section class="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div x-show="source === 'rfq'" x-cloak class="mt-4">
                ...
                <select name="rfq_quote_id" :disabled="source !== 'rfq'" onchange="window.location.href='<?php echo e(route('purchase-orders.create')); ?>?rfq_quote_id='+encodeURIComponent(this.value)" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
                    <option value="">— Select awarded RFQ items —</option>
                    <?php $__currentLoopData = $awardedQuotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $quote): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($quote->id); ?>" <?php echo e($selectedQuote?->id === $quote->id ? 'selected' : ''); ?>><?php echo e($quote->rfq->rfq_number); ?> — <?php echo e($quote->vendor->name); ?> · <?php echo e($quote->items->count()); ?> awarded item<?php echo e($quote->items->count() === 1 ? '' : 's'); ?> · Rs <?php echo e(number_format($quote->items->sum('total'), 2)); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
                <?php if($awardedQuotes->isEmpty()): ?><p class="mt-2 text-xs text-gray-400">No awarded RFQ items are waiting for a Purchase Order.</p><?php endif; ?>
            </div>
        </section>

        <?php if($selectedQuote): ?>
        <section x-show="source === 'rfq'" x-cloak class="rounded-xl border border-teal-200 bg-teal-50 p-6">
            <div class="flex flex-wrap items-start justify-between gap-3"><div><p class="text-xs font-semibold uppercase text-teal-700">Selected supplier</p><h2 class="mt-1 text-lg font-bold text-gray-900"><?php echo e($selectedQuote->vendor->name); ?></h2><p class="mt-1 text-sm text-gray-600">RFQ <?php echo e($selectedQuote->rfq->rfq_number); ?> · only the awarded items below will be ordered.</p></div><p class="font-mono text-lg font-bold text-teal-800">Rs <?php echo e(number_format($selectedQuote->items->sum('total'), 2)); ?></p></div>
            <div class="mt-5 overflow-x-auto rounded-lg border border-teal-100 bg-white"><table class="min-w-full text-sm"><thead class="bg-teal-50 text-left text-xs uppercase text-teal-800"><tr><th class="px-4 py-3">Item</th><th class="px-4 py-3">Request details</th><th class="px-4 py-3 text-right">Qty</th><th class="px-4 py-3 text-right">Rate</th><th class="px-4 py-3 text-right">Total</th></tr></thead><tbody class="divide-y divide-gray-100"><?php $__currentLoopData = $selectedQuote->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><tr><td class="px-4 py-3 font-medium text-gray-800"><?php echo e($item->description); ?></td><td class="px-4 py-3 text-xs text-gray-500"><?php echo e($item->request_remarks ?? '—'); ?></td><td class="px-4 py-3 text-right font-mono"><?php echo e($item->quantity); ?> <?php echo e($item->unit); ?></td><td class="px-4 py-3 text-right font-mono">Rs <?php echo e(number_format($item->unit_rate, 2)); ?></td><td class="px-4 py-3 text-right font-mono">Rs <?php echo e(number_format($item->total, 2)); ?></td></tr><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></tbody></table></div>
        </section>
        <?php endif; ?>
        ...
        <section x-show="source === 'manual'" x-cloak class="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div class="mt-4"><label class="mb-1 block text-sm font-medium text-gray-700">Vendor</label><select name="vendor_id" :disabled="source !== 'manual'" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"><option value="">— Select vendor —</option><?php $__currentLoopData = $vendors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vendor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($vendor->id); ?>" <?php echo e(old('vendor_id') === $vendor->id ? 'selected' : ''); ?>><?php echo e($vendor->name); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div>
...
<div class="col-span-12 rounded-lg bg-teal-50 px-3 py-2 text-sm text-teal-900" x-show="lowestRates(item).length" x-cloak>
    ...
    <p class="mt-1 text-xs">Compare the unit and specification before choosing. Each purchase order is for one vendor. Select the vendor above and enter the suggested rate if suitable.</p>
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-6">
            <h2 class="text-base font-semibold text-gray-800"><?php echo e($selectedQuote ? '2' : '3'); ?>. Delivery and standard terms</h2>
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2"><div class="sm:col-span-2"><label class="mb-1 block text-sm font-medium text-gray-700">Delivery address <span class="text-red-500">*</span></label><textarea name="delivery_address" required rows="2" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"><?php echo e(old('delivery_address', config('kathford.college_address'))); ?></textarea></div><div><label class="mb-1 block text-sm font-medium text-gray-700">Expected delivery date <span class="text-red-500">*</span></label><input type="date" name="expected_delivery_date" required value="<?php echo e(old('expected_delivery_date')); ?>" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"></div><div class="sm:col-span-2"><label class="mb-1 block text-sm font-medium text-gray-700">Terms &amp; conditions <span class="text-red-500">*</span></label><textarea name="terms_and_conditions" required rows="3" placeholder="Delivery, warranty, payment, and acknowledgement terms" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"><?php echo e(old('terms_and_conditions')); ?></textarea></div><div class="sm:col-span-2 rounded-lg border <?php echo e($purchaseOrderChain ? 'border-teal-200 bg-teal-50 text-teal-800' : 'border-amber-200 bg-amber-50 text-amber-800'); ?> px-4 py-3 text-sm"><p class="font-semibold">Purchase Order approval is centrally controlled</p><?php if($purchaseOrderChain): ?><p class="mt-1">This PO will use the administrator-configured chain: <strong><?php echo e($purchaseOrderChain->name); ?></strong>.</p><?php else: ?><p class="mt-1">No active PO approval chain is configured. You may save the PO, but an administrator must configure one in Administration → Approval Chains before submission.</p><?php endif; ?></div></div>
...
<a href="<?php echo e(route('purchase-orders.index')); ?>" class="py-2.5 text-sm text-gray-500">Cancel</a>
<?php endif; ?></div>
    </form>
<?php endif; ?>
</div>
...
function purchaseOrderBuilder() {
    const initialItems = <?php echo json_encode(old('items', []), 512) ?>;
    return {
        approvedRates: <?php echo \Illuminate\Support\Js::from($approvedRates)->toHtml() ?>,
        ...
        source: <?php echo json_encode($rfqMode ? 'rfq' : 'manual', 15, 512) ?>,
        rfqSubtotal: Number(<?php echo json_encode($selectedQuote?->items->sum('total') ?? 0, 15, 512) ?>),
        taxApplied: <?php echo json_encode((bool) old('tax_applied', false), 512) ?>,
        taxRate: Number(<?php echo json_encode(old('tax_rate', 13), 512) ?>) || 13,
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ashbinkumarchamrel/Downloads/kathford-process/resources/views/purchase-orders/create.blade.php ENDPATH**/ ?>