<?php $__env->startSection('title', 'Process Payment'); ?>
<?php $__env->startSection('page-title', 'Process Payment'); ?>
...
?>
<div class="max-w-5xl space-y-5" x-data="paymentSchedule(<?php echo e((float) $payment->amount_due); ?>, <?php echo e((float) $existingTds); ?>, <?php echo e(old('tds_applied', $payment->tds_applied) ? 'true' : 'false'); ?>, <?php echo \Illuminate\Support\Js::from($existingType)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($scheduleRows)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($channels->first()?->id)->toHtml() ?>)">
    <?php if($lockedSchedules->isNotEmpty()): ?>
    ...
    <?php $__currentLoopData = $lockedSchedules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lockedRow): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><p class="mt-2"><?php echo e($lockedRow->schedule_month?->format('M Y')); ?> · Week <?php echo e($lockedRow->schedule_week); ?> · Rs <?php echo e(number_format($lockedRow->net_amount,2)); ?> · <?php echo e(ucfirst(str_replace('_',' ',$lockedRow->status))); ?></p><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <section class="rounded-xl border border-blue-200 bg-blue-50 px-5 py-4">
        <p class="text-xs font-semibold uppercase tracking-wider text-blue-700"><?php echo e($paymentLabel); ?></p>
        <div class="mt-1 flex flex-wrap items-center justify-between gap-3"><div><h1 class="font-mono text-xl font-bold text-blue-900"><?php echo e($paymentReference); ?></h1><p class="mt-1 text-sm text-blue-800"><?php echo e($paymentSubline); ?></p></div><p class="text-xs text-blue-700">Activity: <?php echo e($payment->activity_name ?: $payment->activity_reference ?: 'Not linked'); ?></p></div>
    </section>

    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('payments.edit')): ?>
<form method="POST" action="<?php echo e(route('payments.update', $payment)); ?>" @submit="if (!isReady) { $event.preventDefault(); message = 'Schedule rows must exactly equal the net payable amount.'; }">
        <?php echo csrf_field(); ?>
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-6">
            <div class="grid gap-4 sm:grid-cols-3">
                <div class="rounded-lg bg-slate-50 p-4">
                    ...
                    <?php if($isDirectActivityPayment): ?>
                    ...
                    <?php else: ?>
                        <p class="mt-2 font-mono text-xl font-bold text-slate-900">Rs <?php echo e(number_format($payment->amount_due, 2)); ?></p><p class="mt-1 text-xs text-slate-500"><?php echo e($isChecklistPayment ? 'Locked from the Checklist bill' : 'Gross amount retained for this payment'); ?></p>
                    <?php endif; ?>
                </div>
            ...
            <div class="mt-5"><label class="mb-1.5 block text-sm font-semibold text-gray-700">Notes</label><textarea name="notes" rows="2" class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2.5 text-sm"><?php echo e(old('notes', $payment->notes)); ?></textarea></div>
        </section>
        <div class="mt-4 flex flex-wrap items-center gap-2"><button class="btn-primary" :disabled="!isReady" :class="!isReady && 'cursor-not-allowed opacity-50'">Save payment schedule</button><?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('payments.view')): ?>
<a href="<?php echo e(route('payments.index')); ?>" class="btn-quiet">Cancel</a>
<?php endif; ?></div>
    </form>
<?php endif; ?>
</div>
...
function paymentSchedule(gross, startingRate, startingApplied, startingType, savedRows, defaultChannel) {
    ...
    return {
        ...
        get netPayable() { return Math.round((this.gross - this.tdsAmount - <?php echo json_encode($lockedAmount, 15, 512) ?>) * 100) / 100; },
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ashbinkumarchamrel/Downloads/kathford-process/resources/views/payments/process.blade.php ENDPATH**/ ?>