...
<div class="wrapper">
    <div class="header">
        ...
        <div class="rfq-badge">{{ $rfq->rfq_number }}</div>
    </div>
    <div class="body">
        <p class="greeting">Dear <strong>{{ $quote->vendor?->name }}</strong>,</p>
        <p class="intro">
        ...
        <div class="info-box">
            <div class="info-row"><span class="info-label">RFQ Reference:</span><span class="info-value">{{ $rfq->rfq_number }}</span></div>
            <div class="info-row"><span class="info-label">Submission By:</span><span class="info-value">{{ $rfq->deadline ? $rfq->deadline->format('d M Y') : 'As soon as possible' }}</span></div>
            @if($rfq->notes)
            <div class="info-row"><span class="info-label">Scope:</span><span class="info-value">{{ $rfq->notes }}</span></div>
            @endif
        </div>
        ...
        <div style="text-align:center;margin-bottom:8px">
            <a href="{{ $portalUrl }}" class="btn">Submit Quotation Online</a>
        </div>
        <p class="link-note">Or copy this link: {{ $portalUrl }}</p>
        <p style="font-size:12px;color:#ef4444">⚠ This link is unique to your company and expires in {{ config('kathford.rfq_link_expiry_days') }} days. Do not share it.</p>
    </div>
    <div class="footer">
