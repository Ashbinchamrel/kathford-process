...
<div class="kcard overflow-x-auto"><table class="w-full"><thead><tr><th>Period</th><th>Programme / batch</th><th>Dates</th></tr></thead><tbody>@foreach($periods as $p)<tr><td>{{ $p->name }} · {{ $p->academic_year }}</td><td>{{ $p->programme }} {{ $p->batch }}</td><td>{{ $p->starts_on }} — {{ $p->ends_on }}</td></tr>@endforeach</tbody></table></div>
@endsection
