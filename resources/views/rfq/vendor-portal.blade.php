...
<div class="max-w-4xl mx-auto px-4 py-10">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div>
                ...
                <p class="font-bold text-gray-800 font-mono">{{ $rfq->rfq_number }}</p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="font-medium text-gray-800">{{ $rfq->deadline ? $rfq->deadline->format('d M Y') : 'Open' }}</p>
            </div>
        </div>
        @if($rfq->notes)
    ...
    @endphp
    @if(($quote->status === 'accepted' || $awardedItems->isNotEmpty()) && !($allowEdit ?? false))
        ...
        <p class="font-bold text-green-800 text-lg">{{ $quote->status === 'accepted' ? 'Your quotation has been accepted!' : $awardedItems->count().' item'.($awardedItems->count() === 1 ? ' has' : 's have').' been awarded to your company!' }}</p>
        <p class="text-green-600 text-sm mt-1">{{ $awardedItems->pluck('description')->join(', ') }}. The issued Purchase Order is available from your dashboard.</p>
        </div>
    ...
    @elseif($quote->status === 'submitted' && ! ($allowEdit ?? false))
    ...
    <form method="POST" action="{{ $submitUrl ?? route('rfq.vendor-submit', $quote->vendor_token) }}">
        @csrf
<input type="hidden" name="_fiscal_year_id" value="{{ $workingFiscalYear?->id }}">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            ...
                            <td class="px-5 py-3 text-gray-800 font-medium">
                                ...
                                @if($item->request_remarks)
                                    <span class="mt-1 block whitespace-pre-line text-xs font-normal leading-5 text-slate-500"><span class="font-semibold text-slate-600">Request details:</span> {{ $item->request_remarks }}</span>
                                @endif
                                @if($item->negotiation_message)
                                    <span class="mt-2 block whitespace-pre-line rounded-md border border-amber-200 bg-amber-50 p-2 text-xs font-normal leading-5 text-amber-900"><span class="font-semibold">Negotiation request from Kathford:</span> {{ $item->negotiation_message }}</span>
                                @endif
                            </td>
                            <td class="px-5 py-3 text-gray-500">{{ $item->unit }}</td>
                            <td class="px-5 py-3 text-right font-mono">{{ $item->quantity }}</td>
                            <td class="px-5 py-3">
                                <input type="number" name="items[{{ $item->id }}][unit_rate]" value="{{ old("items.{$item->id}.unit_rate", $item->unit_rate) }}" step="0.01" min="0" required {{ in_array($item->award_status, ['accepted', 'rejected'], true) ? 'readonly' : '' }}
                                       data-quote-rate data-quantity="{{ $item->quantity }}" data-total-target="quote-line-total-{{ $idx }}"
                                       placeholder="0.00"
                            ...
                            <td id="quote-line-total-{{ $idx }}" class="px-5 py-3 text-right font-mono text-gray-700">Rs 0.00</td>
                        </tr>
                        @if($item->award_status === 'accepted')
                        ...
                        @elseif($item->award_status === 'rejected')
                    ...
                    <tfoot class="bg-gray-50">
                        ...
                        <tr>
                            <td colspan="5" class="px-5 py-3 text-right font-semibold text-gray-600"><label class="inline-flex items-center gap-2"><input type="hidden" name="tax_applied" value="0"><input type="checkbox" name="tax_applied" value="1" data-quote-tax-applied @checked(old('tax_applied', $quote->tax_applied))> Apply Tax / VAT</label></td>
                            <td class="px-5 py-2"><label class="text-xs text-gray-500">Rate % <input type="number" name="tax_rate" value="{{ old('tax_rate', $quote->tax_rate ?: 13) }}" step="0.01" min="0" max="100" data-quote-tax-rate class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-right font-mono text-sm"></label></td>
                        </tr>
                        <tr class="bg-teal-50">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-4">
            <div>
                ...
                <input type="text" name="delivery_timeline" value="{{ old('delivery_timeline', $quote->delivery_timeline) }}" placeholder="e.g. 3-5 business days" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Payment Terms</label>
                <input type="text" name="payment_terms" value="{{ old('payment_terms', $quote->payment_terms) }}" placeholder="e.g. 30 days net" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Notes / Remarks</label>
                <textarea name="notes" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none resize-none">{{ old('notes', $quote->notes) }}</textarea>
            </div>
        </div>
