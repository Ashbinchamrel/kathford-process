...
<div class="max-w-2xl mx-auto px-4 py-10">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-8 text-center shadow-sm">
        ...
        <p class="text-gray-500 text-sm mb-6">
            Your quotation for <span class="font-semibold text-gray-700">{{ $quote->rfq->rfq_number }}</span>
        ...
        <div class="bg-gray-50 rounded-xl p-4 text-left text-sm space-y-2 mb-4">
            <div class="flex justify-between">
                ...
                <span class="font-medium text-gray-800">{{ $quote->vendor?->name }}</span>
            </div>
            @if($quote->total_quoted !== null)
            <div class="flex justify-between">
                <span class="text-gray-500">Total Amount</span>
                <span class="font-semibold text-gray-800">NPR {{ number_format($quote->total_quoted, 2) }}</span>
            </div>
            <div class="flex justify-between"><span class="text-gray-500">Tax Amount</span><span class="font-semibold text-gray-800">NPR {{ number_format($quote->tax_amount, 2) }}</span></div>
            <div class="flex justify-between border-t border-gray-200 pt-2"><span class="font-semibold text-gray-700">Grand Total</span><span class="font-bold text-gray-800">NPR {{ number_format($quote->grand_total, 2) }}</span></div>
            @endif
            @if($quote->submitted_at)
            <div class="flex justify-between">
                <span class="text-gray-500">Submitted At</span>
                <span class="text-gray-700">{{ $quote->submitted_at->format('d M Y, g:i A') }}</span>
            </div>
            @endif
        </div>
