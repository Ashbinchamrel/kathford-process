@extends('layouts.app')
@section('title', 'New Activity Form')
@section('page-title', 'New Activity Form')

@section('content')
<div class="max-w-4xl" x-data="formBuilder()">

    @can('activity_forms.create')
<form method="POST" action="{{ route('activity-forms.store') }}" enctype="multipart/form-data" id="activity-form">
        @csrf
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    @error('form_category_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                {{-- Reason field (shown for unplanned categories) --}}
                <div class="sm:col-span-2" x-show="showReason" x-cloak>
                    ...
                    <textarea name="unplanned_reason" rows="3" :required="showReason"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none resize-none">{{ old('unplanned_reason') }}</textarea>
                    @error('unplanned_reason') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                    <textarea name="remarks" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none resize-none">{{ old('remarks') }}</textarea>
                </div>
            </div>
        </div>

        {{-- Section 2: Logistics Table --}}
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th class="px-2 py-1.5 text-left text-xs font-semibold text-gray-500 w-28">Rate (NPR)</th>
                            ...
                            <tr class="border-t border-gray-100">
                                ...
                                <td class="px-2 py-1">
                                    <input type="text" :name="`line_items[${idx}][item_name]`" x-model="item.description"
                                           class="w-full border border-gray-200 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-teal-400 outline-none" required>
                                </td>
                                <td class="px-2 py-1">
                                ...
                                <td class="px-2 py-1">
                                    <input type="text" :name="`line_items[${idx}][unit]`" x-model="item.unit"
                                           placeholder="pcs"
                                           class="w-full border border-gray-200 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-teal-400 outline-none">
                                </td>
                                <td class="px-2 py-1">
                                ...
                                <td class="px-2 py-1 text-right font-medium text-gray-700" x-text="formatNPR(item.amount)"></td>
                                <td class="px-1 py-1">
                    ...
                    <tfoot>
                        <tr class="border-t-2 border-gray-200 bg-gray-50">
                            <td colspan="5" class="px-2 py-1.5 text-right text-sm font-semibold text-gray-700">Total Estimated Amount</td>
                            <td class="px-2 py-1.5 text-right font-bold text-gray-900" x-text="formatNPR(total)"></td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
...
<a href="{{ route('activity-forms.index') }}" class="text-gray-500 hover:text-gray-700 text-sm ml-auto">Cancel</a>
@endcan
        </div>
    </form>
@endcan
...
function formBuilder() {
    return {
        categories: @json($categories->keyBy('id')->map(fn($cat) => ['requires_reason' => $cat->requires_reason, 'requires_logistic_table' => $cat->requires_logistic_table])),
        budgets: @json($budgetOptions),
        categoryId: '{{ old('category_id', '') }}',
        departmentId: @json(old('department_id', $planningBudget?->department_id ?? '')),
        budgetId: @json(old('budget_id', $planningBudget?->id ?? '')),
        showReason: false,
        showLogistics: true,
