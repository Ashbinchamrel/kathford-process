...
<head>
    ...
    <title>@yield('title', 'Sign In') – Kathford Process Portal</title>
...
<body class="h-full flex" style="background:#F4F5F8;">
    ...
    <div class="hidden lg:flex lg:w-5/12 flex-col justify-between p-12" style="background:#1C3557;">
        <div>
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 overflow-hidden rounded-md flex items-center justify-center font-bold text-white text-sm"
                     style="background:#00A99D;">@if($organisationLogo)<img src="{{ asset('storage/'.$organisationLogo) }}" alt="Organisation logo" class="h-full w-full bg-white object-contain p-0.5">@else KI @endif</div>
                <div>
                    <p class="text-white font-semibold text-sm leading-tight">{{ $organisationName }}</p>
        ...
        <p class="text-xs" style="color:#4A6A8A;">
            © {{ date('Y') }} Kathford International College. All rights reserved.
        </p>
    </div>
    ...
    <div class="flex-1 flex items-center justify-center p-8">
        <div class="w-full max-w-sm">
            ...
            <div class="flex items-center gap-3 mb-8 lg:hidden">
                <div class="w-10 h-10 overflow-hidden rounded-md flex items-center justify-center font-bold text-white text-sm"
                     style="background:#1C3557;">@if($organisationLogo)<img src="{{ asset('storage/'.$organisationLogo) }}" alt="Organisation logo" class="h-full w-full bg-white object-contain p-0.5">@else KI @endif</div>
                <div>
                    <p class="text-gray-900 font-semibold text-sm leading-tight">{{ $organisationName }}</p>
            ...
            <p class="text-xs text-center mt-8 text-gray-400 lg:hidden">
                © {{ date('Y') }} Kathford International College
            </p>
        </div>
    </div>
