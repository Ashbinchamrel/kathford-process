...
<head>
    ...
    <title>@yield('title', 'Dashboard') – Kathford Process Portal</title>
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="flex items-center gap-3 px-4 py-4" style="border-bottom:1px solid rgba(255,255,255,0.07);">
            <div class="w-10 h-10 overflow-hidden rounded-md flex items-center justify-center font-bold text-white text-sm flex-shrink-0" style="background:#00A99D;">
                @if($organisationLogo)
                    <img src="{{ asset('storage/'.$organisationLogo) }}" alt="Organisation logo" class="h-full w-full object-contain bg-white p-0.5">
                @else
                    KI
                @endif
            </div>
            <div class="min-w-0">
                <p class="truncate text-white font-semibold text-[13px] leading-tight" title="{{ $organisationName }}">{{ \Illuminate\Support\Str::limit($organisationName, 24) }}</p>
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-2">
            ...
            @can('activity_forms.view')<p class="nav-section">Forms</p>@endcan

            @can('activity_forms.view')
<a href="{{ route('activity-forms.index') }}"
               class="nav-link {{ request()->routeIs('activity-forms.*') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
...
@endcan
            ...
            @if(auth()->user()->canAny(['rfq.view','purchase_orders.view','checklists.view','payments.view','payment_authorisations.view','vendors.view','budgets.view']))
            <p class="nav-section">Procurement</p>

            @can('rfq.view')
<a href="{{ route('rfq.index') }}"
               class="nav-link {{ request()->routeIs('rfq.*') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
...
<a href="{{ route('payment-authorisations.index') }}"
               class="nav-link {{ request()->routeIs('payment-authorisations.*') ? 'active' : '' }}">
...
@endcan
        ...
        <div class="px-3 py-3" style="border-top:1px solid rgba(255,255,255,0.07);">
            <div class="flex items-center gap-3 px-1">
                @if(auth()->user()->avatar)
                    <img src="{{ auth()->user()->avatar }}" class="w-8 h-8 rounded-full object-cover flex-shrink-0" alt="">
                @else
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
                         style="background:#00A99D;">{{ substr(auth()->user()->name, 0, 1) }}</div>
                @endif
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="flex-shrink-0 flex items-center gap-4 px-6 py-0" style="height:52px;background:#fff;border-bottom:1px solid #E5E9EE;">
            ...
            @include('partials.fiscal-year-selector')
            {{-- Right controls --}}
                    ...
                    <button @click="open=!open" @click.away="open=false"
                        ...
                        @else
                            <div class="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold"
                                 style="background:#1C3557;">{{ substr(auth()->user()->name, 0, 1) }}</div>
                        @endif
        ...
        <main class="flex-1 overflow-y-auto p-6">
            @include('partials.fiscal-year-notice')
                @if(request()->routeIs('admin.*') && (auth()->user()->isSuperAdmin() || auth()->user()->can('procurement_setup.manage')))
                    <a href="{{ route('settings.index') }}" class="inline-block text-sm text-teal-700 mb-4">← Back to Settings</a>
                @endif
                @yield('content')
        </main>
    </div>
