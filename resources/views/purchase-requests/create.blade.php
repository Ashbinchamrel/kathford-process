...
<div class="max-w-4xl" x-data="prBuilder()">
    <form method="POST" action="{{ route('purchase-requests.store') }}" enctype="multipart/form-data">
        ...
        {{-- ── Source: Approved Activity Form OR Standalone ── --}}
            ...
            <div class="flex gap-4">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="pr_source" value="activity" id="src-activity" class="text-teal-500"
                           {{ old('pr_source', $activityForm || $approvedActivities->isNotEmpty() ? 'activity' : 'standalone') === 'activity' ? 'checked' : '' }}>
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="pr_source" value="standalone" id="src-standalone" class="text-teal-500"
                           {{ old('pr_source', $activityForm || $approvedActivities->isNotEmpty() ? 'activity' : 'standalone') === 'standalone' ? 'checked' : '' }}>
            ...
            <div id="activity-section">
                ...
                <select name="activity_form_id" id="activity-select"
                    ...
                    @foreach($approvedActivities as $activity)
                        <option value="{{ $activity->id }}" {{ old('activity_form_id', $activityForm?->id) == $activity->id ? 'selected' : '' }}>
                            {{ $activity->form_number }} — {{ $activity->activity_name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-4 space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $activityForm?->activity_name) }}" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none" placeholder="Brief description of what you need">
                    @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Required By</label>
                    <input type="date" name="deadline_date" value="{{ old('deadline_date', $activityForm?->deadline_date?->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Justification / Reason</label>
                    <textarea name="description" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none resize-none">{{ old('description', $activityForm?->remarks) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-4">
            ...
            <p class="text-xs text-gray-400 mt-1">PDF or images, max {{ config('kathford.max_attachment_size_kb') }}KB each.</p>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('purchase-requests.index') }}" class="text-gray-500 hover:text-gray-700 text-sm py-2.5">Cancel</a>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    activitySelect.addEventListener('change', function () {
        if (this.value) {
            window.location.href = '{{ route('purchase-requests.create') }}?activity_form_id=' + encodeURIComponent(this.value);
        }
    });
    updateSource();
});
