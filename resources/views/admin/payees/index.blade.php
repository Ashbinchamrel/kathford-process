...
@section('page-title', 'Payee Information')
...
<div class="mx-auto max-w-7xl space-y-6">
    ...
    <div class="grid gap-6 xl:grid-cols-5">
        <section class="rounded-2xl border border-slate-200 bg-white shadow-sm xl:col-span-3">
            ...
            <form method="POST" action="{{ route('admin.payees.store') }}" class="p-5 sm:p-6" id="payee-form">
                @csrf
                <input type="hidden" name="type" id="payee-type-value" value="{{ old('type') }}">
                <div class="grid gap-5 md:grid-cols-2">
                    <div class="md:col-span-2">
                        ...
                        <input id="payee-name" required name="name" value="{{ old('name') }}" placeholder="e.g. Kathmandu Transport Service" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                    </div>
                    <div>
                        ...
                        <select id="payee-type-selector" required class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                            <option value="" disabled {{ old('type') ? '' : 'selected' }}>Select payee type</option>
                            @foreach($types as $type)
                                <option value="{{ $type }}" {{ old('type') === $type ? 'selected' : '' }}>{{ $type }}</option>
                            @endforeach
                            <option value="__other__" {{ old('type') === '__other__' ? 'selected' : '' }}>Other — add a new type</option>
                        </select>
                        <p class="mt-1.5 text-xs text-slate-500">Choose a standard type or add a new one.</p>
                    </div>
                    <div id="custom-type-wrap" class="{{ old('type') === '__other__' ? '' : 'hidden' }}">
                        ...
                        <input id="custom-type" name="custom_type" value="{{ old('custom_type') }}" placeholder="e.g. Visiting Faculty" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                    </div>
                    ...
                    <div>
                        ...
                        <input id="bank-name" name="bank_name" value="{{ old('bank_name') }}" placeholder="e.g. Nabil Bank" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                    </div>
                    <div>
                        <label for="account-number" class="mb-1.5 block text-sm font-semibold text-slate-700">Account number</label>
                        <input id="account-number" name="account_number" value="{{ old('account_number') }}" inputmode="numeric" autocomplete="off" placeholder="Account number" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                    </div>
                    <div class="md:col-span-2">
                        <label for="account-name" class="mb-1.5 block text-sm font-semibold text-slate-700">Account holder name</label>
                        <input id="account-name" name="account_name" value="{{ old('account_name') }}" placeholder="Name shown on the bank account" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                    </div>
                </div>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white shadow-sm xl:col-span-2">
            ...
            <div class="p-5 sm:p-6">
                <form method="POST" action="{{ route('admin.payment-accounts.store') }}" class="grid gap-3 sm:grid-cols-5">
                    @csrf
                    <div class="sm:col-span-3">
                        ...
                        <input id="payment-account-name" required name="name" value="{{ old('name') }}" placeholder="e.g. Supplier Payment" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-100">
                    </div>
                ...
                <div class="mt-6 space-y-3">
                    ...
                    @forelse($accounts as $account)
                        <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-3">
                            <form method="POST" action="{{ route('admin.payment-accounts.update', $account) }}" class="grid items-end gap-2 sm:grid-cols-5">
                                @csrf @method('PUT')
                                <div class="sm:col-span-3"><label class="mb-1 block text-[11px] font-bold uppercase tracking-wide text-slate-500">Account name</label><input required name="name" value="{{ $account->name }}" class="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-2 text-sm"></div>
                                <div><label class="mb-1 block text-[11px] font-bold uppercase tracking-wide text-slate-500">Code</label><input name="code" value="{{ $account->code }}" class="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-2 text-sm"></div>
                                <div class="flex items-center justify-between gap-2"><label class="inline-flex items-center gap-1.5 text-xs font-medium text-slate-600"><input type="hidden" name="active" value="0"><input type="checkbox" name="active" value="1" @checked($account->active) class="rounded border-slate-300 text-teal-600 focus:ring-teal-500">Active</label><button class="text-sm font-semibold text-teal-700 hover:text-teal-800">Save</button></div>
                            </form>
                        </div>
                    @empty
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-5 py-4 sm:flex sm:items-center sm:justify-between sm:px-6">
            ...
            <span class="mt-2 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600 sm:mt-0">{{ $payees->total() }} total</span>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($payees as $payee)
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-6 py-4 font-semibold text-slate-800">{{ $payee->name }}<p class="mt-1 text-xs font-normal text-slate-500">{{ $payee->account_name ?: 'No account holder entered' }}</p></td>
                            <td class="px-5 py-4"><span class="inline-flex rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">{{ $payee->type }}</span></td>
                            <td class="px-5 py-4 text-slate-700">{{ $payee->bank_name ?: '—' }}<p class="mt-1 font-mono text-xs text-slate-500">{{ $payee->account_number ?: 'No account number' }}</p></td>
                            <td class="px-5 py-4"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $payee->active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">{{ $payee->active ? 'Active' : 'Inactive' }}</span></td>
                            <td class="px-6 py-4 text-right">
                                <details class="text-left"><summary class="inline-flex cursor-pointer rounded-lg px-3 py-2 text-sm font-semibold text-teal-700 hover:bg-teal-50">Edit</summary><form method="POST" action="{{ route('admin.payees.update', $payee) }}" class="mt-3 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">@csrf @method('PUT')<input name="name" value="{{ $payee->name }}" required class="rounded-lg border-slate-300 text-sm"><input name="type" value="{{ $payee->type }}" required class="rounded-lg border-slate-300 text-sm"><input name="bank_name" value="{{ $payee->bank_name }}" placeholder="Bank name" class="rounded-lg border-slate-300 text-sm"><input name="account_number" value="{{ $payee->account_number }}" placeholder="Account number" class="rounded-lg border-slate-300 text-sm"><input name="account_name" value="{{ $payee->account_name }}" placeholder="Account holder name" class="rounded-lg border-slate-300 text-sm"><label class="flex items-center gap-2 text-sm text-slate-600"><input type="hidden" name="active" value="0"><input type="checkbox" name="active" value="1" @checked($payee->active) class="rounded border-slate-300 text-teal-600">Active</label><div class="sm:col-span-2"><button class="rounded-lg bg-teal-600 px-3.5 py-2 text-sm font-semibold text-white hover:bg-teal-700">Save changes</button></div></form></details>
                            </td>
                        </tr>
                    @empty
        ...
        @if($payees->hasPages())<div class="border-t border-slate-100 px-5 py-3 sm:px-6">{{ $payees->withQueryString()->links() }}</div>@endif
    </section>
</div>
