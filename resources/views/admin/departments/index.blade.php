...
<div class="space-y-6">
    ...
    @endif
    @if($errors->has('error'))
    <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">{{ $errors->first('error') }}</div>
    @endif
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-3 font-mono text-xs text-gray-500">{{ $dept->code }}</td>
                        <td class="px-5 py-3 text-gray-600">{{ $dept->head?->name ?? '—' }}</td>
                        <td class="px-5 py-3 text-gray-500">{{ $dept->users_count }}</td>
                        ...
                        <td class="px-5 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $dept->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                {{ $dept->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-5 py-3 text-right space-x-3">
                            <a href="{{ route('admin.departments.edit', $dept) }}" class="text-teal-600 hover:text-teal-700 text-sm font-medium">Edit</a>
                            @if(! $isProtected)
                            <form method="POST" action="{{ route('admin.departments.destroy', $dept) }}" class="inline" onsubmit="return confirm('Delete this department? This cannot be undone.')">
                                @csrf @method('DELETE')
        ...
        <form method="POST" action="{{ route('admin.departments.store') }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" required maxlength="10" style="text-transform:uppercase"
