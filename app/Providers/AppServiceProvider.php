...
class AppServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->scoped(\App\Support\FiscalYearContext::class);
    }
    ...
    public function boot(): void
    {
        ...
        Gate::before(function ($user, $ability, array $arguments = []) {
            $year = app(\App\Support\FiscalYearContext::class);
            if ($year->enabled && !$year->writable() && in_array($ability, ['update', 'delete', 'verify', 'finalApprove'], true)
                && isset($arguments[0]) && is_object($arguments[0]) && method_exists($arguments[0], 'fiscalYear')) return false;
            ...
            if ($year->enabled && !$year->writable() && in_array($parts[0], $transactionModules, true)
                && !in_array($parts[1] ?? '', ['view', 'export'], true)) return false;
